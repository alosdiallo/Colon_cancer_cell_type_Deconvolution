...
```{r}
combinded_data <- readRDS(file = "/Users/adiallo/Desktop/Thesis/Data_Documents/Single_cell_samples/Single Cell/combined_data.rds")
```
...
# Re-create the CellChat object if necessary
cellchat_mets <- createCellChat(object = metastatic_data, meta = mets_meta, group.by = "labels")


# Compute communication probabilities using triMean
cellchat_mets <- computeCommunProb(cellchat_mets, type = "triMean")

nonmets_meta$labels = droplevels(factor(nonmets_meta$labels), exclude = setdiff(levels(factor(nonmets_meta$labels),unique(nonmets_meta$labels))))
