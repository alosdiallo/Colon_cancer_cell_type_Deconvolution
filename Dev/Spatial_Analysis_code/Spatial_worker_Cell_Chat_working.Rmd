...
```
```{r}
plotGeneExpression(cellchat, signaling = "GAS", enriched.only = TRUE, type = "violin")
```
```{r}
...
gene_expression_data <- FetchData(seurat_object, vars = c("MYC", "ACTA2"))
...
# Plot TNF
p1 <- ggplot(plot_data, aes(x = x, y = y, color = MYC)) +
...
# Plot IFNG
p2 <- ggplot(plot_data, aes(x = x, y = y, color = ACTA2)) +
  geom_point(alpha = 0.6, size = 1) +
...
ggplot(plot_data, aes(x = x, y = y)) +
  geom_point(aes(color = MYC), alpha = 0.6, size = 1) +
  scale_color_gradient(low = "white", high = "red") +
  new_scale("color") +  # Add new color scale for the next geom
  geom_point(aes(color = ACTA2), alpha = 0.6, size = 1) +
  scale_color_gradient(low = "white", high = "blue") +
  coord_fixed() +
  theme_minimal() +
