...
```{r}
library(limma)
tcga.M.val <- ifelse(tcga.beta.val >= 1,1-1E-6,ifelse(tcga.beta.val <= 0,1E-6,tcga.beta.val))
tcga.M.val <- log(tcga.M.val)-log(1-tcga.M.val)

tcga.X.cell <- model.matrix(~any_mets+age+sex+MLH1+Tumor+Epithelial+Endothelial+Stromal+Treg,data=df.cell) # note: add more cell types!
# %>% filter(!Distant_Mets)
tcga.X.nocell <- model.matrix(~any_mets+age+sex+MLH1,data=df.cell)
block <- NULL

# !!
tcga.corfit <- duplicateCorrelation(tcga.M.val[1:350000,], tcga.X.cell, block=block, ndups=2)

tcga.fit <- lmFit(tcga.M.val,tcga.X.cell,block=block,correlation=tcga.corfit$consensus)
tcga.eb <- eBayes(tcga.fit)
tcga.res <- topTable(tcga.eb,coef=2,adjust="BH",n=Inf)
tcga.res[1:10,]
```
